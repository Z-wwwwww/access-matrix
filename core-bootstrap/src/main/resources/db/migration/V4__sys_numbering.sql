-- V4: numbering tables + back-fill columns on existing pms_auth_user (idempotent)

ALTER TABLE pms_auth_user ADD COLUMN IF NOT EXISTS user_no VARCHAR(32);
ALTER TABLE pms_auth_user ADD COLUMN IF NOT EXISTS email VARCHAR(255);
ALTER TABLE pms_auth_user ADD COLUMN IF NOT EXISTS display_name VARCHAR(128);
...
COMMENT ON TABLE sys_numbering_management IS 'Numbering definition (code_kbn = category)';
COMMENT ON COLUMN sys_numbering_management.recycle_division IS '0=none, 1=daily, 2=monthly, 3=yearly';
COMMENT ON COLUMN sys_numbering_management.format_sentence IS 'Template using [%K], [%D], [%] placeholders';
...
COMMENT ON TABLE sys_numbering_key IS 'Numbering counter buckets per (tenant, category, key)';

-- Seed: USER numbering definition (format U + 8-digit seq, no recycle)
INSERT INTO sys_numbering_management
