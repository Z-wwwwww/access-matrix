...
- Drain runs on **one pod at a time**: the poll takes the `core_job_lock` row `outbox-dispatcher::system` via the same `JobLockService` every `ScheduledJob` uses. Without it, the `pod×2~N` topology in `docs/deployment.md` has every pod selecting the same `dispatch_state = 0` rows (the row is written back only *after* the sink returns), so a real sink would see every event N times on the happy path — not the crash-only duplication `EventDispatchSink`'s contract promises. Pinned by `OutboxDispatcherClusterTest`.
- Config: `app.outbox.enabled` (default true) / `poll-interval-ms` (5000) / `batch-size` (200) / `max-attempts` (5) / `lock-lease-seconds` (60 — how long a pod that dies mid-batch keeps the lock before another takes over).
