...
- **租户管理员账号(=超管用户)在用户管理界面完全只读**:对**其他任何用户**(无论持有什么用户管理权限)整账号不可操作——编辑(含邮箱/显示名)/改角色/改部门/改状态/删除/重置密码/强制登出全部拒绝;管理员**只能本人**在「个人资料」页改自己的邮箱/显示名(`updateOwnProfile`)。识别走后端 `UserDto.View.superAdmin`(以及内置 admin 的 `builtin`)标志位下发,前端 `User.vue` 据此整行置灰按钮 + 提示原因。后端硬守卫:`UserAdminService` 各变更方法统一走 `assertNotProtectedAdmin`(`error.user.adminProtected`);该守卫同时**取代了旧的「最后一个超管」守卫**——唯一超管整账号动不了,租户恒有一个可用超管。(回归:此前 `update` 仅锁部门/状态、邮箱/显示名对任何 user:update 持有者开放,且 `changeDept` 完全没拦超管。)
...
- [ ] **租户管理员账号完全只读**:用其他用户(持 user:update 等用户管理权限)登录,用户列表里超管那行(带「租户管理员」徽标)的 编辑/重置密码/停用/删除/强制登出 按钮**全部置灰**(hover 提示原因);绕过前端直接调任一 `/admin/user/*` 写接口(update/assign-roles/change-dept/status/delete/reset-password/force-logout)针对超管 → 一律 `error.user.adminProtected`(含改邮箱/显示名)。超管本人在「个人资料」页可改自己的邮箱/显示名。普通用户不受影响。识别用后端下发的 `superAdmin`/`builtin` 标志,不再硬编码用户名。
...
- [ ] **内置 admin 同样完全只读**:内置 `demo-admin` 那行与租户超管同等保护——编辑按钮置灰、所有写接口 `error.user.adminProtected`。判定由后端 `UserDto.View.builtin` 下发(后端 `BUILTIN_ADMIN_USERNAME` 单一来源),不在前端硬编码用户名;后端守卫兜底。
