...
| 路径 | OIDC 模式行为 | 原因 |
|---|---|---|
| 用户管理页 → "重置密码"按钮 | 正常可用（`POST /admin/user/{id}/reset-password`） | 不再是旧的本地 hash 写入：它在 Keycloak 中轮换一次性临时密码（`temporary=true`），踢下线，并把临时密码一次性回显——与平台运维用户管理同一流程。受保护管理员（内置 / 租户超管）与自己那行会被拒绝。 |
