...
### 后端 → Keycloak 管理凭据（自动 bootstrap 的 service account）

ops 新建租户时，后端要调 Keycloak Admin REST API 建 realm + 首个管理员。后端对**所有**这类调用都以一个专用 **service-account client**（`access-matrix-provisioner`）、用 `client_credentials` 认证 —— **一条路径，dev 与 prod 完全一致，运行时没有用户名/密码**。见 `KeycloakAdminClientFactory.runtimeClient()`。

**这个 provisioner client 在启动时自动创建**，由 `KeycloakProvisionerSeeder` 完成 —— 无需在 KC 控制台点任何按钮。启动时（`app.keycloak.bootstrap.enabled=true`）它用一次性 bootstrap 管理凭据：

1. 创建 `access-matrix-provisioner` client（confidential + service account 开），并把它的 secret 同步成 `CORE_KEYCLOAK_PROVISIONER_SECRET`；
2. 给它的 service account 授 `master` realm 角色 **`create-realm`**；
3. 给每个**既有的**被管 realm（`demo`、`system`）授 **`manage-users`/`view-users`** —— 它们是导入的、不是 provisioner 建的，所以 KC 的 create-realm 自动授权覆盖不到。

之后 provisioner 自己建的 realm（acme…），KC 会自动授管理权，无需额外配置。

**为什么用 service-account client（而非 `master` admin 用户）：** 运行时凭据是机器 secret、**最小权限**（create-realm + 已知 realm 的 manage，而非无所不能的 `master` `admin`），不受人类账号生命周期影响（密码策略/过期/"必须改密码"/MFA —— 也就是 admin 密码漂移导致的 "invalid_grant 400"），可程序化轮换，泄露后影响**有界且可一键吊销**（删/换一个 client），而不是 IdP 根的全面沦陷。
...
# 一次性 bootstrap（复用你现有的 KC root admin）：
CORE_KEYCLOAK_BOOTSTRAP_ENABLED=true
CORE_KEYCLOAK_BOOTSTRAP_USERNAME=<KC root admin>
CORE_KEYCLOAK_BOOTSTRAP_PASSWORD=<KC root 密码>
CORE_KEYCLOAK_BOOTSTRAP_MANAGED_REALMS=demo,system
```

启动一次 → 看日志 `[kc-provisioner] provisioner ready …`。然后**收尾加固**：

```bash
# provisioner client 已持久化在 KC —— 撤掉一次性 root 凭据
unset CORE_KEYCLOAK_BOOTSTRAP_USERNAME CORE_KEYCLOAK_BOOTSTRAP_PASSWORD
CORE_KEYCLOAK_BOOTSTRAP_ENABLED=false
```

稳态：后端持有的 KC secret 只剩 `CORE_KEYCLOAK_PROVISIONER_SECRET` 一个。

**轮换：** 常规轮换 = 换 secret。改 `CORE_KEYCLOAK_PROVISIONER_SECRET`，临时把 bootstrap 打开跑一次（它会把 client 的 secret 重同步成新值），再关掉 —— 或在 KC 控制台改 secret 再同步 env。换 secret **不影响** client 对象、service account、以及所有角色授权。

**注意（仅 DR/迁移）：** 若 provisioner *client* 被删、或你新起一套 KC，重建的 client 是新身份，不持有 KC 之前自动授给老 client 的"已建租户 realm"的管理权 —— 需重跑 bootstrap 并对那些 realm 重新授权。常规换 secret 不会触发这个。

**卫生：** provisioner secret 放 secrets manager（Vault / AWS Secrets Manager / sealed secret）。要最强姿态，用 mTLS 或 signed-JWT 客户端认证（`private_key_jwt`）替代共享 secret。
