...
// Drawer/Dialog 内の場合は自分より高い z-index を貰う（Select と同じ仕組み）
...
function updatePosition() {
  ...
  applyAbsolutePopupPosition(panelRef.value, rect, {
    // 高さは max-h と合うように 400 を渡し、上下空間判定を正確にする
    assumedHeight: 400,
...
<template>
  <div class="relative">
    ...
    <!-- Dropdown — Teleport to body so it can escape the Drawer's overflow clip -->
    <Teleport to="body">
      <Transition name="picker">
        <div
          v-if="open"
          ref="panelRef"
          class="fixed -top-[9999px] -left-[9999px] z-[60] w-[520px] max-h-[400px] bg-card border border-border rounded-xl shadow-xl flex flex-col overflow-hidden"
        >
          <!-- Search -->
